...
def run_gmaps_scraper_docker_core(
    *, keywords_list, city_name_key, depth_from_ui, extract_emails_flag,
    ...
    try:
        ...
        log_q_streamlit.put(f"CMD_DOCKER:{cmd_str_for_log}")
        ...
        for stdout_line in iter(process.stdout.readline, ''):
            ...
            if clean_line:
                ...
                logger_instance.debug(f"GOSOM ({city_name_key}): {clean_line}")
            
            if st.session_state.get('stop_scraping_flag', False):
                logger_instance.warning(f"DETENER para {city_name_key}. Terminando Docker...")
                log_q_streamlit.put(f"INFO:Intentando detener Docker para {city_name_key}...")
                process.terminate()
            ...
            if time.time() - last_row_count_update_time > 2: # Chequear cada 2 segundos
                if os.path.exists(raw_fp_host):
                    try:
                        ...
                        log_q_streamlit.put({"type": "LIVE_ROW_COUNT", "city": city_name_key, "count": current_rows})
        ...
        if return_code == 0:
            ...
            if os.path.exists(raw_fp_host):
                ...
                log_q_streamlit.put({"type": "LIVE_ROW_COUNT", "city": city_name_key, "count": final_rows}) # Última actualización
                if final_rows > 0:
            ...
            log_q_streamlit.put(f"ERROR:Docker falló {city_name_key}. Código: {return_code}")
            return None
    except FileNotFoundError:
        logger_instance.critical("Comando 'docker' no encontrado.", exc_info=True)
        log_q_streamlit.put("ERROR_CRITICAL_THREAD:Comando 'docker' no encontrado.")
        return None
    except Exception as e:
        logger_instance.critical(f"Excepción en Docker exec ({city_name_key}): {e}", exc_info=True)
        log_q_streamlit.put(f"ERROR_CRITICAL_THREAD:Excepción Docker: {str(e)[:100]}")
        return None
    finally:
