...
# shlex no es estrictamente necesario si solo usamos ' '.join() para loguear el comando
# import shlex 
...
def run_gmaps_scraper_docker_core(
    ...
    gmaps_coords_dict,     
    language_code,         
    default_config_depth,  # Esta es la profundidad del archivo parameters_default.json
    results_prefix,        
    logger_instance
):
    ...
    lat = city_info.get('latitude') # Coincide con tu parameters_default.json
    lon = city_info.get('longitude')# Coincide con tu parameters_default.json
    ...
    try:
        ...
        logger_instance.success(f"Archivo temp de queries: {temp_queries_filepath_host} ({len(keywords_list)} kw).")

        with open(raw_output_filepath_host, 'w', encoding='utf-8') as f: pass 
        logger_instance.success(f"Archivo CSV vacío creado en host: {raw_output_filepath_host}")
        
        # Usar rutas absolutas para los volúmenes de Docker
        abs_config_dir_path = os.path.abspath(config_dir_path)
        abs_raw_csv_folder_path = os.path.abspath(raw_csv_folder_path)
        logger_instance.debug(f"Ruta absoluta para config volume (host): {abs_config_dir_path}")
        logger_instance.debug(f"Ruta absoluta para raw data volume (host): {abs_raw_csv_folder_path}")
...
def transform_gmaps_data_core(df_raw, city_key_origin, logger_instance):
    ...
    df_processed = df[relevant_columns].copy() # Crea el df con las columnas relevantes en el orden deseado
...
def process_city_data_core(city_key, keywords_list, depth_from_ui, extract_emails_flag, config_params, paths_config, logger_instance):
    ...
    default_config_depth = config_params.get('default_depth', 1) 
    ...
    if not raw_csv_path or not os.path.exists(raw_csv_path):
        logger_instance.error(f"No se generó CSV crudo para {city_key} o no se encontró. No se puede transformar.")
        return pd.DataFrame(), raw_csv_path # Devolver el path aunque esté vacío o no exista, para logueo en UI

    # 2. Cargar el archivo CSV crudo
    df_raw = pd.DataFrame() 
    try:
        # Leer el CSV. Si está vacío pero existe, pd.read_csv devolverá un DF vacío si names se especifica.
        df_raw = pd.read_csv(
            raw_csv_path, 
            header=None, 
            names=gmaps_column_names, 
            encoding='utf-8',
            on_bad_lines='warn', # 'skip' o 'warn'
            low_memory=False
        )
        logger_instance.success(f"CSV crudo cargado para {city_key}: {len(df_raw)} filas, {len(df_raw.columns)} columnas.")
        
        # Verificación adicional si se leyó un DF vacío de un archivo no vacío (raro con names)
        if df_raw.empty and os.path.getsize(raw_csv_path) > 0:
             logger_instance.warning(f"CSV crudo {raw_csv_path} no está vacío pero se leyó como DataFrame vacío. Revisar delimitadores, contenido o lista 'gmaps_column_names'.")

    except pd.errors.EmptyDataError: # Se da si el archivo está completamente vacío y no se dan nombres de columnas
         logger_instance.warning(f"Archivo CSV crudo {raw_csv_path} está completamente vacío (EmptyDataError).")
         # Crear un DataFrame vacío con las columnas esperadas para que la transformación no falle
        ...
        return pd.DataFrame(), raw_csv_path # Devolver path para logueo en UI
    ...
    return df_transformado, raw_csv_path # Devolver también el raw_csv_path